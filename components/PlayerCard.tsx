        ...
        <CardContent className='pt-5'>
            ...
            <div className='flex items-center justify-between w-full'>
                {props.nextMatches?.map((match: string, index) => (
                    <div className={`w-full flex flex-row justify-center p-1 text-xs ${convertColorDifficulty(props.nextMatchesDiff[index])}`} key={index}>{match.split(' ')[0]} {match.split(' ')[1]}</div>
                ))}
            </div>
        </CardContent>
